...
export default function StatisticsDashboard() {
  ...
  const costLabel = provider === 'codex' ? 'Not applicable' : data?.summary.apiEquivalentCost === null ? 'Unavailable' : `$${data?.summary.apiEquivalentCost.toFixed(2)}`;
        ...
        {loading ? <Skeleton /> : data && (
          <>
            <section aria-label="Usage summary" className="mt-6 grid overflow-hidden rounded-xl border border-[#1e1e2e] bg-[#111118] sm:grid-cols-3 lg:grid-cols-6">
              ...
              <Metric label="API-equivalent" value={costLabel} detail="Estimated, not billed" icon={CircleDollarSign} />
              <Metric label="Codex credits" value={data.summary.codexCredits === null ? 'Unavailable' : data.summary.codexCredits.toFixed(1)} detail="Official rate card estimate" icon={Activity} />
            ...
            <section className="mt-4 overflow-hidden rounded-xl border border-[#1e1e2e] bg-[#111118]">
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[820px] text-left">
                  ...
                  <tbody>{data.sessions.slice(0, sessionLimit).map((session) => <tr key={session.id} className="border-b border-[#1e1e2e]/70 text-[10px] text-slate-500 last:border-b-0 hover:bg-[#16161f]"><td className="whitespace-nowrap px-5 py-3 font-mono">{new Intl.DateTimeFormat('en-GB', { timeZone: 'Asia/Dubai', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(session.timestamp))}</td><td className="px-3 py-3 capitalize"><span className={`inline-flex rounded-full px-2 py-1 text-[9px] ${session.provider === 'claude' ? 'bg-violet-500/10 text-violet-300' : 'bg-emerald-500/10 text-emerald-300'}`}>{session.provider}</span></td><td className="max-w-40 truncate px-3 py-3 text-slate-400" title={session.project}>{session.project}</td><td className="max-w-44 truncate px-3 py-3 font-mono" title={session.model}>{session.model}</td><td className="px-3 py-3 text-right font-mono tabular-nums text-slate-300">{tokenFormatter.format(session.totalTokens)}</td><td className="px-3 py-3 text-right font-mono tabular-nums">{formatPercent(session.totalTokens ? session.cachedTokens / session.totalTokens : null)}</td><td className="px-3 py-3 text-right font-mono tabular-nums">{formatDuration(session.durationMs)}</td><td className="px-5 py-3 text-right font-mono tabular-nums">{session.codexCredits !== null ? `${session.codexCredits.toFixed(1)} cr` : session.apiEquivalentCost !== null ? `$${session.apiEquivalentCost.toFixed(2)}` : '—'}</td></tr>)}</tbody>
                </table>
              </div>
